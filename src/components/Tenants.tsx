...
import { Plus, Edit2, Trash2, Eye, Search, X, Phone, Calendar, Receipt, Zap, Droplets, Wifi, Flame, Wrench, FileText, CheckCircle, Building2, Clock, IndianRupee, TrendingUp, TrendingDown, AlertCircle, ChevronRight, ArrowUpRight, ArrowDownRight, History, CircleDollarSign } from 'lucide-react';
...
const emptyTenant = (): Omit<Tenant, 'id' | 'avatarColor'> => ({
  ...
  leaseStart: new Date().toISOString().split('T')[0], leaseEnd: '', idProof: '', emergencyContact: '', notes: '', status: 'Active' as TenantStatus, dueDay: 1,
});
...
export default function Tenants({ tenants, setTenants, properties, bills, setBills, payments, settings, showAddModal, setShowAddModal, filterType = 'active' }: Props) {
  ...
  const openEdit = (t: Tenant) => {
    setForm({ name: t.name, email: t.email, phone: t.phone, propertyId: t.propertyId, room: t.room, rent: t.rent, deposit: t.deposit, leaseStart: t.leaseStart, leaseEnd: t.leaseEnd, idProof: t.idProof, emergencyContact: t.emergencyContact, notes: t.notes, status: t.status, dueDay: t.dueDay || 1 });
  ...
  const save = () => {
    ...
    if (editingId) setTenants(tenants.map(t => t.id === editingId ? { ...t, ...form, room: finalRoom } : t));
    else setTenants([...tenants, { id: generateId(), avatarColor: getAvatarColor(), ...form, room: finalRoom }]);
    setIsFormOpen(false);
  };
  const remove = (id: string) => { setTenants(tenants.filter(t => t.id !== id)); setBills(bills.filter(b => b.tenantId !== id)); };
  ...
  const saveBill = () => { if (!billTenantId || !billForm.description.trim()) return; setBills([...bills, { id: generateId(), tenantId: billTenantId, ...billForm }]); setShowBillForm(false); };
  const markBillPaid = (billId: string) => { setBills(bills.map(b => b.id === billId ? { ...b, status: 'Paid' as BillStatus, paidDate: new Date().toISOString().split('T')[0] } : b)); };
  const deleteBill = (billId: string) => { setBills(bills.filter(b => b.id !== billId)); };
  ...
  const getTenantStats = (tenantId: string) => {
    ...
    // Split pending into OVERDUE (past due date) vs UPCOMING (future due date)
    ...
    return { totalPaid, overdue, upcoming, nextDuePayment, billsPaid, billsPending, lastPaid, paymentCount: tp.length, billCount: tb.length };
  };

  // Grand totals
  const grandStats = useMemo(() => {
    const today = new Date().toISOString().split('T')[0];
    const activeTenantIds = baseFiltered.map(t => t.id);
    const totalRent = baseFiltered.reduce((s, t) => s + t.rent, 0);
    const totalCollected = payments.filter(p => activeTenantIds.includes(p.tenantId) && p.status === 'Paid').reduce((s, p) => s + p.amount, 0);
    const unpaid = payments.filter(p => activeTenantIds.includes(p.tenantId) && p.status !== 'Paid');
    const totalOverdue = unpaid.filter(p => p.dueDate && p.dueDate < today).reduce((s, p) => s + (p.dueAmount - p.amount), 0);
    const totalUpcoming = unpaid.filter(p => !p.dueDate || p.dueDate >= today).reduce((s, p) => s + (p.dueAmount - p.amount), 0);
    const totalBillsPending = bills.filter(b => activeTenantIds.includes(b.tenantId) && b.status === 'Pending').reduce((s, b) => s + b.amount, 0);
    return { totalRent, totalCollected, totalOverdue, totalUpcoming, totalBillsPending };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{filterType === 'old' ? 'Inactive/past tenants' : `${baseFiltered.length} tenant${baseFiltered.length !== 1 ? 's' : ''} • ${formatCurrency(grandStats.totalRent)}/month receivable`}</p>
        </div>
      ...
      {filterType !== 'old' && baseFiltered.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
            ...
            <div><p className="text-xs text-gray-500">Total Collected</p><p className="text-lg font-bold text-green-600">{formatCurrency(grandStats.totalCollected)}</p></div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
            <div className="p-2.5 bg-red-100 rounded-xl"><TrendingDown className="w-5 h-5 text-red-600" /></div>
            <div><p className="text-xs text-gray-500">Overdue</p><p className="text-lg font-bold text-red-600">{formatCurrency(grandStats.totalOverdue)}</p></div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
            <div className="p-2.5 bg-orange-100 rounded-xl"><Receipt className="w-5 h-5 text-orange-600" /></div>
            <div><p className="text-xs text-gray-500">Bills Pending</p><p className="text-lg font-bold text-orange-600">{formatCurrency(grandStats.totalBillsPending)}</p></div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
            <div className="p-2.5 bg-indigo-100 rounded-xl"><CircleDollarSign className="w-5 h-5 text-indigo-600" /></div>
            <div><p className="text-xs text-gray-500">Monthly Receivable</p><p className="text-lg font-bold text-indigo-600">{formatCurrency(grandStats.totalRent)}</p></div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-3 bg-white p-3 rounded-xl shadow-sm border border-gray-100">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name or phone..." className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" />
        </div>
        <select value={filterProperty} onChange={e => setFilterProperty(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
              ...
              <div className="p-5">
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex gap-0.5">
                    ...
                    <button onClick={() => openAddBill(t.id)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-green-600 transition"><Receipt className="w-4 h-4" /></button>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className={`truncate ${t.propertyId ? 'text-gray-700' : 'text-red-500 italic'}`}>{getPropertyName(t.propertyId)}</span>
                  </div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div className={`flex items-center gap-1.5 p-2 rounded-lg ${hasOverdue ? 'bg-red-50' : isDueSoon ? 'bg-amber-50' : 'bg-indigo-50'}`}>
                    ...
                    <span className={`text-xs font-medium ${hasOverdue ? 'text-red-700' : isDueSoon ? 'text-amber-700' : 'text-indigo-600'}`}>
                      {getOrdinal(t.dueDay || 1)} monthly
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-3 space-y-2">
                  {stats.overdue > 0 && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-red-600 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Rent overdue</span>
                      <span className="text-sm font-semibold text-red-600">{formatCurrency(stats.overdue)}</span>
                    </div>
                  )}
                  {stats.upcoming > 0 && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-indigo-600 flex items-center gap-1"><Clock className="w-3 h-3" /> Next due ({formatDate(nextDueDate)})</span>
                      <span className="text-sm font-semibold text-indigo-600">{formatCurrency(stats.upcoming)}</span>
                    </div>
                  )}
                  {stats.upcoming === 0 && stats.overdue === 0 && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-500">Rent</span>
                      <span className="text-sm font-semibold text-gray-800">{formatCurrency(t.rent)}</span>
                    </div>
                  )}
                  {stats.billsPending > 0 && (
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-orange-600">+ Bills pending</span>
                      <span className="text-sm font-semibold text-orange-600">{formatCurrency(stats.billsPending)}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                    <span className="text-sm font-bold text-gray-700">{totalOwed > 0 ? 'Total Due' : 'Monthly Rent'}</span>
                ...
                <div className="flex items-center justify-between mt-3 pt-2">
                  <button onClick={() => { setShowDetail(t); setDetailTab('payments'); }} className="flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-700 font-medium transition">
                    <History className="w-3.5 h-3.5" /> {stats.paymentCount} payments <ChevronRight className="w-3 h-3" />
                  </button>
                  <button onClick={() => remove(t.id)} className="p-1 rounded hover:bg-red-50 text-gray-300 hover:text-red-500 transition"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            </div>
          );
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 text-gray-400">
          <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><Building2 className="w-8 h-8 text-gray-300" /></div>
          <p className="text-lg font-medium">{filterType === 'old' ? 'No old tenants' : 'No tenants yet'}</p>
          <p className="text-sm mt-1">{filterType === 'old' ? 'Inactive tenants will appear here' : 'Add your first tenant to get started'}</p>
        </div>
      )}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 text-white/80 text-sm">
                        <Building2 className="w-3.5 h-3.5" /> {getPropertyName(showDetail.propertyId)} {showDetail.room && `• Room ${showDetail.room}`}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-5">
                {detailTab === 'info' && (
                  <div className="grid grid-cols-2 gap-4">
                    {[['Phone', showDetail.phone, Phone], ['Email', showDetail.email, null], ['Due Day', `${getOrdinal(showDetail.dueDay || 1)} of every month`, Calendar], ['Join Date', formatDate(showDetail.leaseStart), Calendar], ['Lease End', formatDate(showDetail.leaseEnd), Calendar], ['Rent', formatCurrency(showDetail.rent), IndianRupee], ['Deposit', formatCurrency(showDetail.deposit), IndianRupee], ['ID Proof', showDetail.idProof, null], ['Emergency', showDetail.emergencyContact, Phone], ['Status', showDetail.status, null]].map(([label, val]) => (
                ...
                {detailTab === 'payments' && (
                  <div className="space-y-2">
                    {tenantPayments.length === 0 ? <p className="text-center text-gray-400 py-8">No payments recorded</p> :
                    tenantPayments.map(p => (
                      <div key={p.id} className={`flex items-center justify-between p-3 rounded-xl ${p.status === 'Paid' ? 'bg-emerald-50' : p.status === 'Overdue' ? 'bg-red-50' : 'bg-amber-50'}`}>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium text-gray-900">{p.status === 'Paid' ? `Paid via ${p.method}` : `Due: ${formatDate(p.dueDate)}`}</p>
                            <p className="text-xs text-gray-500">{p.date ? formatDate(p.date) : 'Pending'} {p.receiptNo && `• ${p.receiptNo}`}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className={`text-sm font-bold ${p.status === 'Paid' ? 'text-emerald-700' : 'text-red-600'}`}>{formatCurrency(p.status === 'Paid' ? p.amount : p.dueAmount - p.amount)}</p>
                          <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold ${p.status === 'Paid' ? 'bg-emerald-100 text-emerald-700' : p.status === 'Overdue' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{p.status}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      tenantBills.map(bill => {
                        ...
                        return (
                          <div key={bill.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                            <div className="flex items-center gap-3">
                              <div className={`p-2 rounded-lg ${BILL_COLORS[bill.type] || 'bg-gray-100 text-gray-600'}`}><BIcon className="w-4 h-4" /></div>
                              <div><p className="text-sm font-medium text-gray-900">{bill.description}</p><p className="text-xs text-gray-500">{bill.type} • Due: {formatDate(bill.dueDate)}</p></div>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-bold">{formatCurrency(bill.amount)}</span>
                              {bill.status === 'Pending' ? (
                                <><button onClick={() => markBillPaid(bill.id)} className="p-1.5 rounded-lg hover:bg-green-100 text-green-600"><CheckCircle className="w-4 h-4" /></button>
                                <button onClick={() => deleteBill(bill.id)} className="p-1.5 rounded-lg hover:bg-red-100 text-red-500"><Trash2 className="w-4 h-4" /></button></>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setIsFormOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Phone</label><input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" placeholder="Phone number" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Email</label><input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" placeholder="email@example.com" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Assign to Property</label><select value={form.propertyId} onChange={e => setForm({ ...form, propertyId: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"><option value="">Select Property</option>{properties.map(p => <option key={p.id} value={p.id}>{p.name} ({p.type})</option>)}</select></div>
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">ID Proof</label><input value={form.idProof} onChange={e => setForm({ ...form, idProof: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" placeholder="Aadhar / PAN" /></div>

...
import { Home, Mail, Lock, Eye, EyeOff, LogIn, UserPlus, ArrowLeft, ShieldCheck } from 'lucide-react';
...
export default function Login({ onLogin }: Props) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        const isOwner = data.user.user_metadata?.role === 'Owner';
        if (!isApproved && !isOwner) {
          setError('Your account is pending approval from the Owner. Please contact the administrator.');
          setIsLoading(false);
          return;
        }
        onLogin({
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) { setError(error.message); setIsLoading(false); return; }
      if (data.user) {
        setSuccess('Account request submitted! The Owner will review and approve your access. You will be notified once approved.');
        setMode('login');
        setPassword('');
      }
    } catch (err: any) {
            ...
            <>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white transition">
              ...
              <div className="mt-6 text-center">
                <p className="text-slate-400 text-sm">
                  ...
                  <button onClick={() => { setMode('signup'); setError(''); setSuccess(''); }} className="text-indigo-400 hover:text-indigo-300 font-medium">Request Access</button>
                </p>
              </div>
            </>
          ) : (
            <>
              <button onClick={() => { setMode('login'); setError(''); setSuccess(''); }} className="flex items-center gap-2 text-slate-400 hover:text-white transition mb-4">
                <ArrowLeft className="w-4 h-4" /> Back to login
              </button>
              <h2 className="text-2xl font-bold text-white mb-2">Request Access</h2>
              <p className="text-slate-400 mb-2">Create an account — requires owner approval</p>
              <div className="flex items-center gap-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl mb-5">
                <ShieldCheck className="w-5 h-5 text-amber-400 shrink-0" />
                <p className="text-xs text-amber-300">New accounts need approval from the Owner before you can login.</p>
              </div>
              ...
              <form onSubmit={handleSignup} className="space-y-5">
                ...
                <button type="submit" disabled={isLoading}
                  className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3.5 rounded-xl font-semibold hover:from-indigo-500 hover:to-purple-500 transition shadow-lg shadow-indigo-500/30 flex items-center justify-center gap-2 disabled:opacity-70">
                  {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><ShieldCheck className="w-5 h-5" />Request Access</>}
                </button>
              </form>
            </>
          )}
